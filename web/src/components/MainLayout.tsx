...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#F5F7FB] text-primary font-sans relative overflow-hidden">
      ...
      {/* Sidebar with Colors */}
      <aside className={cn(
        "bg-white border-r border-gray-100 transition-all duration-500 ease-in-out z-20 shadow-[20px_0_50px_rgba(0,0,0,0.02)]",
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="p-10 flex items-center justify-between">
            {isSidebarOpen ? (
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-black flex items-center justify-center text-white font-black text-2xl rounded-[14px] shadow-2xl rotate-3">S</div>
                <div className="flex flex-col">
                    <span className="text-2xl font-black tracking-tighter leading-none">SCHOLAR</span>
                    <span className="text-[9px] font-black uppercase tracking-[0.3em] text-accent mt-1">Management Pro</span>
                </div>
              </div>
            ) : (
                <div className="w-12 h-12 bg-black flex items-center justify-center text-white font-black text-2xl rounded-[14px] mx-auto shadow-xl">S</div>
            )}
          ...
          {/* Navigation */}
          <nav className="flex-1 overflow-y-auto py-8 px-6 space-y-10 scrollbar-hide">
            {filteredGroups.map((group, idx) => (
              <div key={idx}>
                {isSidebarOpen && (
                  <div className="flex items-center space-x-3 mb-6 px-4">
                      <div className="h-2 w-2 rounded-full bg-accent/20"></div>
                      <h3 className="text-[10px] font-black text-[#9E9E9E] uppercase tracking-[0.4em]">
                        {group.group}
                      </h3>
                  </div>
                )}
                    ...
                    return (
                      <li key={itemIdx}>
                        <Link
                          to={item.path}
                          className={cn(
                            "flex items-center p-4 rounded-[18px] transition-all group relative",
                            isActive
                        ...
                        >
                          ...
                          <item.icon size={22} className={cn(isSidebarOpen ? "mr-4" : "mx-auto", isActive && "text-accent")} />
                          {isSidebarOpen && (
                            <span className="flex-1 font-black text-xs uppercase tracking-tight">{item.title}</span>
                          )}
                          {isSidebarOpen && isActive && <ChevronRight size={16} className="text-accent" />}
                        </Link>
                      </li>
                    );
                  })}
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-12 z-10 shadow-sm">
          <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-8">
            <div className="hidden md:flex items-center space-x-4 bg-gray-50 px-6 py-3 rounded-full border border-gray-100">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-[#9E9E9E]">Session Active</span>
                <span className="text-[10px] font-black uppercase tracking-widest text-black">{selectedYear?.libelleAnneeScolaire}</span>
            </div>

            <div className="w-14 h-14 bg-white border-4 border-gray-50 flex items-center justify-center font-black text-black rounded-[20px] shadow-2xl cursor-pointer hover:border-accent hover:rotate-6 transition-all group overflow-hidden">
